...
import com.example.jetfilms.Models.Pagination.SerialsPagingSource
...
class SeriesRepository @Inject constructor(private val apiService: ApiInterface) {
    ...
    fun getPaginatedSerials(getResponse: suspend (page: Int) -> SeriesPageResponse, pagesLimit: Int = Int.MAX_VALUE) =
        Pager(
            config = PagingConfig(
                pageSize = PAGE_SIZE,
            ),
